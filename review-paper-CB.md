...
- The recognized journal names are:
  ...
  - **Computer Science Conference Proceedings**: `ICWSM`
  - **Writing for Public Audience** `Op-Eds`
...
| `diff` | Apply all auto-fixable changes, then open VS Code for review. If git-tracked, tell user to use Source Control (Cmd+Shift+G) for per-hunk revert. If not, create `.pre-sub-backup` and open `code -d backup file`. |
| `apply` | Apply all auto-fixable changes without opening VS Code. For headless or non-VS-Code environments. |

If a token matches a fix mode (`review`, `diff`, `apply`), store it as `FIX_MODE`. If no fix mode is found, default to `report` (read-only — no files are modified).

Store the resolved target journal as `TARGET_JOURNAL` and fix mode as `FIX_MODE` for use in the report and fix gate.
...
| # | File | Agent | Fix | Severity | Status |
|---|---|---|---|---|---|
| 1 | ... | 1 | Short description | CRITICAL/MAJOR/MINOR | Applied / Skipped / Failed |
...
*If fixes were applied, mark fixed items as "DONE" and focus on remaining items.*
...
5. How many issues were flagged in each category (counts)
...
Build a **fix plan** from the consolidated report, including only items where a concrete text change can be specified.

**Auto-fixable categories (by agent):**

*Agent 1 — Spelling, Grammar & Style:*
- Spelling errors → correct spelling
- Grammar errors (subject-verb agreement, article usage, comma splices) → fix
- Style violations ("interestingly", "importantly", passive voice, tautologies) → rewrite
- Typographic inconsistencies (hyphenation, em-dash vs en-dash) → standardize
- Number formatting inconsistencies → standardize

*Agent 2 — Internal Consistency:*
- Numerical inconsistencies where the table is authoritative → fix text to match table
- Terminology drift → standardize to the first-defined usage
- Broken cross-references (`\ref{}` pointing to wrong label) → fix

*Agent 4 — Mathematics & Notation:*
- LaTeX math formatting (missing `\left`/`\right`, `*` → `\cdot`, text in math mode) → fix
- Notation inconsistencies (same symbol for different things) → standardize
- Undefined notation → add `% TODO: define [symbol]` comment at first use

*Agent 5 — Tables & Figures:*
- Missing significance star definitions in table notes → add standard definition
- Missing cross-references (figure/table defined but never cited) → flag with `% TODO` comment

**Not auto-fixable (recommend only):**
- Unsupported claims and causal language (Agent 3) — requires author judgment
- Contribution and framing issues (Agent 6) — requires substantive revision
- Missing analyses or robustness checks — requires new work
- Mathematical or derivation errors — requires domain expertise to correct

For each fixable item, record: file path, line number, current text, proposed replacement, reason, severity, and source agent.
...
2. Display: severity, source agent, file and line, current text, proposed fix, and reason.
...
> You are applying reviewed pre-submission fixes to an academic paper. Each fix has been vetted by six review agents and a consolidation step. Apply them exactly as specified using the Edit tool. Do not alter substance — only fix spelling, grammar, formatting, consistency, and notation as specified.
>
...
## Phase 4: Reproduce Report as .tex for Overleaf

Take the report as produced in .md form and produce also as a .tex for Overleaf file